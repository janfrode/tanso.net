...
%if %(if [ -f "/etc/httpd/build/special.mk" ]; then echo 1 ; elif [ -f "/usr/lib/httpd/build/special.mk" ]; then echo 2 ; else echo -1; fi)
	%define _apacheroot /etc/httpd
%else 
...
test -f /sbin/restorecon && restorecon /etc/httpd/modules/mod_security2.so
